            ...
            <div className='flex flex-col items-center pb-10'>
                ...
                <div className="flex flex-col items-center pb-10">
                    <div className="flex flex-row items-center">
                        ...
                        <select name="color" id="color" onChange={(e) => setColor(e.target.value)} className="text-white rounded-lg mt-2 font-bold uppercase bg-gray-950 p-2 placeholder: seleccionar">
                            {colors.map((color) => (
                    ...
                    <button onClick={() => handleCreate(color)} className={`w-full p-2 bg-gray-950 text-white rounded-lg mt-2 font-bold uppercase duration-200 hover:bg-gray-800`}>Crear</button>
                </div>
            </div>
            <div>
                <ul className="flex flex-col items-center">
                    {contadores.map((contador) => (
                        <li key={contador.id} className="flex flex-row items-center space-x-4 rounded-lg mt-2 font-bold uppercase" style={{ color: contador.color }}>
                            <button className="font-bold text-3xl" onClick={() => handleDelete(contador.id)}> X</button>
